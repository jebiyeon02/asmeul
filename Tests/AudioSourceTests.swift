...
@main struct AudioSourceTests {
  static func main() {
    ...
    let sources = groupedAudioSources(applications: apps, records: [main, helper, other, similar, shared, helper], selected: 0)
    precondition(sources.count == 2)
    precondition(sources.first { $0.id == 100 }?.processIDs == [10, 11])
    ...
    print("PASS: app/helper grouping, source isolation, deduplication, silence, helper restart and app exit")
  }
}
