...
double lateTail(float space){
 ASMEULDSP d;d.targetSpace=space;d.targetWarmth=0;d.targetGain=.7;d.prepare(48000);auto p=d.parameters();float l,r;
...
double toneEnergy(float warmth){
 ASMEULDSP d;d.targetSpace=0;d.targetWarmth=warmth;d.targetGain=.7;d.prepare(48000);auto p=d.parameters();float l,r;double energy=0;
...
int main(){
 ...
 ASMEULDSP orbit;orbit.targetSpace=0;orbit.targetWarmth=0;orbit.targetOrbit=1;orbit.prepare(48000);auto op=orbit.parameters();float ol,orr;double ratio=1;
 ...
 ASMEULDSP neutral;neutral.targetSpace=neutral.targetWarmth=neutral.targetOrbit=0;neutral.targetGain=.7;neutral.prepare(48000);auto np=neutral.parameters();
 ...
 std::cout<<"PASS: long tail ratio "<<longTail/shortTail<<", warmth HF ratio "<<dark/clear<<", orbit channel ratio "<<ratio<<", neutral transparency\n";
    ...
    for(double rate:{44100.,48000.,96000.})for(float space:{.1f,.4f,1.f}){
        ASMEULDSP dsp;dsp.prepare(rate);dsp.targetSpace=space;dsp.targetOrbit=.5f;
